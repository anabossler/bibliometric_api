...
WHERE
  ...
  AND (
    toLower(p.abstract) CONTAINS 'supramolecular'
    OR toLower(p.abstract) CONTAINS 'host guest'
    OR toLower(p.abstract) CONTAINS 'host–guest'
    OR toLower(p.abstract) CONTAINS 'molecular recognition'
    OR toLower(p.abstract) CONTAINS 'self assembly'
    OR toLower(p.abstract) CONTAINS 'self-assembly'
    OR toLower(p.abstract) CONTAINS 'supramolecular assembly'
    OR toLower(p.abstract) CONTAINS 'supramolecular polymer'
    OR toLower(p.abstract) CONTAINS 'supramolecular chemistry'
    OR toLower(p.abstract) CONTAINS 'cucurbit'
    OR toLower(p.abstract) CONTAINS 'cucurbituril'
    OR toLower(p.abstract) CONTAINS 'calixarene'
    OR toLower(p.abstract) CONTAINS 'pillararene'
    OR toLower(p.abstract) CONTAINS 'cyclodextrin'
    OR toLower(p.abstract) CONTAINS 'molecular self-assembly'
  )
  AND NOT toLower(p.abstract) CONTAINS 'plastic'
  AND NOT toLower(p.abstract) CONTAINS 'recycling'
  AND NOT toLower(p.abstract) CONTAINS 'sustainability'
  AND NOT toLower(p.abstract) CONTAINS 'supply chain'
  AND NOT (
    toLower(p.title) =~ '.*(clinical|patient|therapy|in vivo|in vitro|disease).*'
    OR toLower(p.abstract) =~ '.*(clinical|patient|therapy|in vivo|in vitro|disease).*'
  )
RETURN
  p.doi AS doi,
...
def compute_citation_insularity(neo: Neo4jClient, df: pd.DataFrame, labels: np.ndarray, outdir: str):
    """Calcula % de citas internas vs externas por cluster"""
    ...
    for c in sorted(np.unique(labels)):
        ...
        # Internal citations (both src and dst in cluster)
    ...
    log.info(f"✓ Citation insularity guardada")
