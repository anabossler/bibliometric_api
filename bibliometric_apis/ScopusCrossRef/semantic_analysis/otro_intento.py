...
class Neo4jClient:
    ...
    def fetch(self, cypher: str, params: Optional[Dict[str, Any]] = None) -> pd.DataFrame:
        """Ejecuta un Cypher y devuelve DataFrame con deduplicado y limpieza."""
        params = params or {}
        ...
        for c in ["doi", "eid", "title", "abstract", "year", "citedBy", "keywords"]:
            if c not in df.columns:
                df[c] = None

        # Mantener solo las que existan en el resultado
        keep_cols = ["doi", "eid", "title", "abstract", "year", "citedBy", "keywords"]
        df = df[[c for c in keep_cols if c in df.columns]]

        # Deduplicado robusto por DOI o título+year
        ...
        df_null["title_norm"] = (
            df_null["title"].fillna("").astype(str)
        ...
        kept = (
            df[df["abstract_len_words"] >= min_words]
            .drop(columns=["abstract_len_words"])
        ...
        try:
            ...
            if yy.notna().any():
                log.info(
                    f"Años en corpus: {int(yy.min()) if yy.notna().any() else 'NA'}.."
                    f"{int(yy.max()) if yy.notna().any() else 'NA'} | n={len(kept)}"
                )
        except Exception:
            pass
...
def cypher_for_keyword_cluster(terms: List[str]) -> str:
    if not terms:
        log.warning("⚠️ Lista de términos vacía en cypher_for_keyword_cluster")
        return """
        MATCH (p:Publication)
        WHERE false
        RETURN p.doi AS doi, p.title AS title, p.abstract AS abstract,
               p.year AS year, p.journal AS journal,
               [] AS keywords, COALESCE(toInteger(p.citedBy),0) AS citedBy
        """
    return f"""
    WITH {json.dumps([t.lower() for t in terms])} AS terms
    ...
    WHERE p.abstract IS NOT NULL AND trim(p.abstract) <> ''
      ...
      AND ANY(t IN terms WHERE toLower(k.name) CONTAINS t)

    // limpiar keywords
    OPTIONAL MATCH (p)-[:HAS_KEYWORD]->(kw:Keyword)
    WITH p, j, collect(DISTINCT kw.name) AS raw_kw
    UNWIND raw_kw AS kw
    WITH p, j, split(kw, "|") AS parts
    UNWIND parts AS part
    WITH p, j, trim(part) AS kw_clean
    WHERE kw_clean <> ""
    RETURN p.doi AS doi, p.eid AS eid, p.title AS title,
           p.abstract AS abstract, p.year AS year, j.name AS journal,
           collect(DISTINCT kw_clean) AS keywords,
...
class Analyzer:
    ...
    def embed(self):
        ...
        # === Construcción de full_text enriquecido ===
        def kw_join(x):
            if isinstance(x, list):
                return " ".join([str(k) for k in x if isinstance(k, str)])
            elif pd.isna(x):
                return ""
            return str(x)

        df = self.df.copy()
        df["kw_text"] = df["keywords"].apply(kw_join) if "keywords" in df.columns else ""
        df["journal_text"] = df["journal"].fillna("") if "journal" in df.columns else ""
        ...
        # === Embeddings principales (abstracts + title + journal + kw_text) ===
        if backend == "tfidf":
            vec = TfidfVectorizer(
                ngram_range=(1, 2),
                min_df=2,
        ...
        elif backend in {"sbert", "specter2", "chemberta"}:
        ...
        # === Keyword embeddings con n-grams ===
        if "kw_text" in df.columns and df["kw_text"].str.strip().any():
            kw_vec = TfidfVectorizer(
                ngram_range=(1, 3),  # unigrams, bigrams y trigrams
                token_pattern=r"[A-Za-z0-9_\-]+",
                min_df=1
            )
            X_kw = kw_vec.fit_transform(df["kw_text"].tolist()).toarray()
            X_kw = X_kw / (np.linalg.norm(X_kw, axis=1, keepdims=True) + 1e-12)
        else:
            log.warning("No keywords disponibles, usando vector vacío")
            X_kw = np.zeros((len(texts), 1))

        # === Combinación ponderada (textos + keywords) ===
        wkw = float(os.getenv("WEIGHT_KEYWORDS", "0.3"))
        alpha, beta = (1.0 - wkw), wkw

        if X_text.shape[0] != X_kw.shape[0]:
            log.error("Mismatch entre abstracts y keywords")
            raise ValueError("Length mismatch in abstracts vs keywords")

        X_text = X_text * alpha
        X_kw = X_kw * beta
        self.X = np.hstack([X_text, X_kw])
...
def parse_args():
    ...
    p.add_argument("--weight-kw", type=float, default=0.3, help="Weight of keyword vectors [0-1]")
    return p.parse_args()

if __name__ == "__main__":
    args = parse_args()

    # --- Habilitar peso híbrido abstracts + keywords ---
    os.environ["WEIGHT_KEYWORDS"] = str(args.weight_kw)
