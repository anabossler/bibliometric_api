...
- NUEVO: Grid search híbrido texto+keywords con bootstrap ARI y stopwords empíricas de dominio.
"""
...
from sklearn.preprocessing import normalize, MultiLabelBinarizer  # NUEVO
...
# Stopwords empíricas validadas del análisis de co-ocurrencia (NUEVO)
EMPIRICAL_DOMAIN_STOPWORDS = {
    # Nivel 1: Omnipresentes (5/5 clusters)
    "circular economy", "plastics waste", "recycling", "waste management",
    "environmental benefits", "end of lives", "plastics", "end of life",
    # Nivel 2: Muy frecuentes (4/5 clusters)
    "mechanical recycling", "plastic packaging", "recycling process",
    "chemical recycling", "plastic waste", "waste plastic",
    # Nivel 3: Metodológicos genéricos
    "life cycle assessment", "lca", "life cycle analysis",
    # Nivel 4: Descriptores vagos
    "environmental performance", "environmental issues", "environmental aspects",
    "recycled materials", "sustainable materials", "environmental monitoring",
}
...
def clean_abstract(t: str, strip_domain_stopwords: bool = False) -> str:
    """
    Limpia abstract. Si strip_domain_stopwords=True, elimina frases de EMPIRICAL_DOMAIN_STOPWORDS.
    """
    ...
    if strip_domain_stopwords:
        s_lower = s.lower()
        for phrase in sorted(EMPIRICAL_DOMAIN_STOPWORDS, key=len, reverse=True):
            s_lower = s_lower.replace(phrase, " ")
        s = s_lower
...
# ========= Ejecutores principales (by-cypher y full-corpus) =========
...
# ========= NUEVO: pipeline híbrido texto+keywords =========
def fetch_corpus_with_keywords(neo: Neo4jClient, min_year=None, max_year=None):
    """
    Fetch corpus completo con keywords del grafo para clustering híbrido.
    Incluye limpieza de abstracts con strip de stopwords empíricas.
    """
    where_clauses = [
        "p.abstract IS NOT NULL", "p.abstract <> ''",
        "p.year IS NOT NULL", "p.year <> ''"
    ]
    if min_year: where_clauses.append(f"toInteger(p.year) >= {int(min_year)}")
    if max_year: where_clauses.append(f"toInteger(p.year) <= {int(max_year)}")
    where_block = " AND ".join(where_clauses)

    cypher = f"""
    MATCH (p:Publication)-[:HAS_KEYWORD]->(k:Keyword)
    WHERE {where_block}
    WITH p, COLLECT(DISTINCT toLower(k.name)) AS keywords
    WHERE SIZE(keywords) >= 2
    RETURN p.doi AS doi, p.eid AS eid, p.title AS title,
           p.abstract AS abstract, p.year AS year,
           COALESCE(toInteger(p.citedBy), 0) AS citedBy,
           keywords
    ...
    df_valid = df.loc[mask].drop_duplicates(subset=["doi_norm"])
    df_null = df.loc[~mask].copy()
    df_null["title_norm"] = df_null["title"].fillna("").astype(str).str.lower().str.replace(r"\s+"," ",regex=True).str.strip()
    df_null = df_null.drop_duplicates(subset=["title_norm"])
    df = pd.concat([
        df_valid.drop(columns=["doi_norm"], errors="ignore"),
        df_null.drop(columns=["doi_norm","title_norm"], errors="ignore")
    ], ignore_index=True)

    # Limpiar abstracts con stopwords de dominio
    df["abstract"] = df["abstract"].fillna("").apply(lambda x: clean_abstract(x, strip_domain_stopwords=True))
    df = df[df["abstract"].str.split().apply(len) >= 20].reset_index(drop=True)

    # One-hot de keywords
    mlb = MultiLabelBinarizer(sparse_output=False)
    X_kw = mlb.fit_transform(df["keywords"])

    log.info(f"Corpus: {len(df)} papers, {X_kw.shape[1]} unique keywords")
    return df, X_kw, mlb.classes_

def bootstrap_stability_hybrid(X, k, n_bootstrap=5, seed=42):
    """Bootstrap ARI para estabilidad (híbrido)."""
    n = X.shape[0]
    rng = np.random.default_rng(seed)
    labels_list = []

    for _ in range(n_bootstrap):
        idx = np.sort(rng.choice(n, size=int(0.9 * n), replace=False))
        X_boot = X[idx]
        km = KMeans(n_clusters=k, n_init=20, random_state=seed)
        labels_boot = km.fit_predict(X_boot)
        labels_list.append((idx, labels_boot))
    ...
    if not aris:
        return float('nan'), (float('nan'), float('nan'))
    return float(np.median(aris)), (float(np.percentile(aris, 25)), float(np.percentile(aris, 75)))

def grid_search_hybrid_weights(X_text, X_kw, weight_ratios, k_range=(4,6), n_bootstrap=5):
    """
    Grid search sobre pesos text:keywords con bootstrap.
    Devuelve DataFrame ordenado por score compuesto.
    """
    results = []

    for w_kw in weight_ratios:
        log.info(f"\n=== Testing weight ratio 1:{w_kw} (text:keywords) ===")

        # Combinar
        X_combined = np.hstack([X_text * 1.0, X_kw * w_kw])
        ...
        for k in range(k_range[0], k_range[1] + 1):
            ...
            ch = calinski_harabasz_score(X_combined, labels)

            # Bootstrap
            ari_median, ari_iqr = bootstrap_stability_hybrid(X_combined, k, n_bootstrap)
            ...
            results.append({
                ...
                'imbalance': max(sizes.values()) / min(sizes.values())
            })

            log.info(f"sil={sil:.3f}, ari={ari_median:.3f}, sizes={sizes}")
    ...
    # Score compuesto
    df_results['score'] = (
        df_results['silhouette'] * 0.3 +
        df_results['ari_median'].fillna(0) * 0.4 +
        (df_results['ch'] / df_results['ch'].max()) * 0.2 -
        (df_results['db'] / df_results['db'].max()) * 0.1
    )

    return df_results.sort_values('score', ascending=False)

def run_full_corpus_grid_search(
    backend: str,
    kmin: int,
    kmax: int,
    outroot: str,
    ...
    """
    Ejecuta grid search de pesos abstracts:keywords con stopwords de dominio
    y lanza el análisis completo con la mejor configuración.
    """
    neo = Neo4jClient()

    try:
        log.info("Fetching corpus with keywords...")
        df, X_kw, keyword_names = fetch_corpus_with_keywords(neo, min_year, max_year)

        if df.empty or X_kw is None:
            log.error("No data")
            return

        log.info(f"Papers: {len(df)}, Keywords: {X_kw.shape[1]}")
        log.info(f"Top 20 keywords: {list(keyword_names[:20])}")

        # Embeddings SBERT (abstracts YA limpios de stopwords)
        log.info("Computing embeddings...")
        model_name = os.getenv("EMBEDDING_MODEL", "sentence-transformers/all-MiniLM-L6-v2")
        if SentenceTransformer is None:
            raise RuntimeError("sentence-transformers no instalado para backend transformer")
        ...
        # Normalizar keywords
        X_kw_norm = normalize(X_kw, norm='l2')

        # Grid search
        log.info("\n=== GRID SEARCH ===")
        weight_ratios = [1, 2, 3, 4, 5, 6, 7, 8, 9]
        results = grid_search_hybrid_weights(
            X_text=X_text,
            X_kw=X_kw_norm,
        ...
        # Guardar
        outdir = os.path.join(outroot, "grid_search_results")
        os.makedirs(outdir, exist_ok=True)
        results.to_csv(os.path.join(outdir, "weight_grid_results.csv"), index=False)

        if results.empty:
            log.error("No hubo resultados en grid search")
            return

        # Mejor config
        best = results.iloc[0]
        log.info(f"\n✓ BEST: ratio={best['ratio']}, k={int(best['k'])}, score={best['score']:.3f}")

        # Ejecutar análisis completo con mejor config
        log.info("\nRunning full analysis with best config...")
        X_best = np.hstack([X_text * 1.0, X_kw_norm * float(best['w_kw'])])
        X_best = normalize(X_best, norm='l2')

        # Clustering final auto con k fijo = best.k usando kmeans para coherencia
        k_final = int(best['k'])
        labels = cluster_kmeans(X_best, k_final, random_state=42)

        # Preparar outputs estándar
        topics = label_topics_c_tfidf([" ".join(tokenize(a)) for a in df["abstract"].tolist()], labels, top_n=12)
        reps   = representative_docs(X_best, df, labels, top_m=5)
        tl     = cluster_timeline(df, labels)
        outdir_final = os.path.join(outroot, f"hybrid_best_k{k_final}_r{best['ratio']}")
        os.makedirs(outdir_final, exist_ok=True)
        export_tables("hybrid_best", df, labels, topics, reps, tl, outdir_final)
        linkM  = centroid_cosine_matrix(X_best, labels)
        trends = enhanced_timeline_trends(tl)
        # Métricas internas
        sil, db, ch = eval_internal(X_best, labels, metric="cosine")
        # No hacemos bootstrap extra aquí (ya evaluado en grid)
        scores = ModelScores(k=k_final, algo="kmeans_hybrid", labels=labels,
                             silhouette=float(sil), db=float(db), ch=float(ch),
                             ari_median=float(best['ari_median']), ari_iqr=(float(best['ari_q25']), float(best['ari_q75'])),
                             cluster_sizes=_cluster_sizes(labels))
        write_validation_report("hybrid_best", scores, linkM, trends, outdir_final)
        save_plot_2d(X_best, labels, os.path.join(outdir_final,"clusters_pca.png"), title=f"Hybrid best — PCA")
        write_semantic_report("hybrid_best", df, labels, topics, reps, outdir_final, method_str=f"kmeans_hybrid(k={k_final}, ratio={best['ratio']})")
        # Insights
        clus_metrics = per_cluster_metrics(X_best, df, labels, trends)
        insights_df  = classify_clusters(clus_metrics, ari_global=scores.ari_median)
        write_business_insights("hybrid_best", insights_df, outdir_final)
        # Config
        write_config(outdir_final, {
            "mode": "grid_search_hybrid",
...
# ========= NUEVO: Semi-supervisado con seeds =========
def assign_by_keyword_dominance(df, cluster_defs, threshold=0.8):
    """
    Asigna papers a clusters si >threshold% de sus keywords pertenecen a una categoría.
    Devuelve -1 para frontera.
    """
    ...
    for idx, row in df.iterrows():
        ...
        matches = {cid: 0 for cid in range(len(cluster_defs))}
        for cid, (_, terms) in enumerate(cluster_defs.items()):
            term_set = [t.lower() for t in terms]
            for kw in kw_list:
                if any(t in kw.lower() for t in term_set):
                    matches[cid] += 1

        total = sum(matches.values())
        if total > 0:
            best_cid = max(matches, key=matches.get)
            if matches[best_cid] / total >= threshold:
                labels[idx] = best_cid
    return labels


def hierarchical_seed_clustering(df, X_text, X_kw, cluster_defs, w_kw=3.0, threshold=0.75, seed=42):
    """
    1. Núcleos por keywords (hard cores).
    2. Frontera: clusterizada con centroides de núcleos como init.
    """
    from sklearn.cluster import KMeans
    from sklearn.preprocessing import normalize

    hard_labels = assign_by_keyword_dominance(df, cluster_defs, threshold)
    X_combined = np.hstack([X_text, X_kw * w_kw])
    X_combined = normalize(X_combined, norm="l2")

    k = len(cluster_defs)
    centroids = np.zeros((k, X_combined.shape[1]))
    for c in range(k):
        mask = (hard_labels == c)
        if mask.sum() > 0:
            centroids[c] = X_combined[mask].mean(axis=0)
        else:
            centroids[c] = X_combined[np.random.randint(len(X_combined))]

    frontier = (hard_labels == -1)
    if frontier.sum() > 0:
        km = KMeans(n_clusters=k, init=centroids, n_init=1, random_state=seed)
        all_labels = km.fit_predict(X_combined)
        final_labels = hard_labels.copy()
        final_labels[frontier] = all_labels[frontier]
    else:
        final_labels = hard_labels

    return final_labels, X_combined, (hard_labels != -1).sum()


def run_semisupervised_clustering(
    backend: str,
    outroot: str,
    ...
    """Pipeline principal semi-supervisado"""
    neo = Neo4jClient()
    try:
        log.info("Fetching corpus with keywords...")
        df, X_kw, keyword_names = fetch_corpus_with_keywords(neo, min_year, max_year)
        ...
        model_name = os.getenv("EMBEDDING_MODEL", "sentence-transformers/all-MiniLM-L6-v2")
        st = SentenceTransformer(model_name)
        X_text = st.encode(df["abstract"].tolist(), show_progress_bar=True, normalize_embeddings=True)
        X_kw_norm = normalize(X_kw, norm='l2')

        labels, X_combined, n_hard = hierarchical_seed_clustering(
            df, X_text, X_kw_norm, BUSINESS_CLUSTER_DEFS,
            w_kw=w_kw, threshold=threshold
        )
        ...
        log.info(f"Métricas: sil={sil:.3f}, DB={db:.3f}, CH={ch:.1f}")
        log.info(f"Núcleos duros: {n_hard}/{len(df)}")

        name = f"semisupervised_t{int(threshold*100)}_w{int(w_kw)}"
        outdir = os.path.join(outroot, name)
        os.makedirs(outdir, exist_ok=True)

        topics = label_topics_c_tfidf([" ".join(tokenize(a)) for a in df["abstract"].tolist()], labels, top_n=12)
        reps = representative_docs(X_combined, df, labels, top_m=5)
        tl = cluster_timeline(df, labels)
        export_tables(name, df, labels, topics, reps, tl, outdir)
        linkM = centroid_cosine_matrix(X_combined, labels)
        trends = enhanced_timeline_trends(tl)

        scores = ModelScores(
            k=len(BUSINESS_CLUSTER_DEFS),
            algo="semisupervised",
            labels=labels,
            silhouette=float(sil), db=float(db), ch=float(ch),
            ari_median=float('nan'), ari_iqr=(float('nan'), float('nan')),
            cluster_sizes=_cluster_sizes(labels)
        )
        write_validation_report(name, scores, linkM, trends, outdir)
        write_semantic_report(name, df, labels, topics, reps, outdir,
                              f"semisupervised(th={threshold}, w_kw={w_kw})")
...
def parse_args():
    ...
    p.add_argument("--grid-search", action="store_true", help="Run grid search for optimal text:keyword weights (hybrid)")
    p.add_argument("--semisupervised", action="store_true", help="Run semi-supervised clustering with keyword seeds")  
    p.add_argument("--seed-threshold", type=float, default=0.75, help="Keyword dominance threshold (default 0.75)")    
    p.add_argument("--w-keywords", type=float, default=3.0, help="Keyword weight (default 3.0)")  
    return p.parse_args()

if __name__ == "__main__":
    args = parse_args()

    # === Grid search híbrido texto+keywords ===
    if args.grid_search:
        os.makedirs(args.outdir, exist_ok=True)
        run_full_corpus_grid_search(
            backend=args.backend,
            kmin=args.kmin,
            kmax=args.kmax,
            outroot=args.outdir,
            bootstrap=args.bootstrap if args.bootstrap else 5,
    ...
    if args.semisupervised:
        os.makedirs(args.outdir, exist_ok=True)
        run_semisupervised_clustering(
            backend=args.backend,
            outroot=args.outdir,
            threshold=args.seed_threshold,
            w_kw=args.w_keywords,
            min_year=args.min_year,
            max_year=args.max_year
        )
        raise SystemExit(0)
    ...
    if not args.by_cypher:
        log.error("Use --by-cypher y --clusters para ejecutar por clúster, o --full-corpus / --grid-search.")
        raise SystemExit(1)
