...
# Tamaños OPTIMIZADOS
PAGE_SIZE   = int(os.getenv("EXPORT_BATCH_SIZE", "100"))
...
def build_filter_clause(search_terms: List[str], require_all: bool = True) -> str:
    """
    Construye la cláusula WHERE para filtrar por términos.
    
    Args:
        search_terms: Lista de términos a buscar (ej: ["circular economy", "plastic"])
        require_all: True = AND (requiere TODOS los términos), False = OR (requiere al menos uno)
    """
    if not search_terms:
        return ""
    ...
    for term in search_terms:
        ...
        conditions.append(
            f"(toLower(p.title) CONTAINS '{term_lower}' OR toLower(p.abstract) CONTAINS '{term_lower}')"
        )
    
    operator = " AND " if require_all else " OR "
    return " AND " + operator.join(conditions)

def get_count_query(search_terms: List[str] = None, require_all: bool = True) -> str:
    filter_clause = build_filter_clause(search_terms, require_all) if search_terms else ""
    return f"""
    MATCH (p:{PAPER_LABEL})
    WHERE p.title IS NOT NULL{filter_clause}
    RETURN count(p) AS total
    """

def get_paged_query(search_terms: List[str] = None, require_all: bool = True) -> str:
    filter_clause = build_filter_clause(search_terms, require_all) if search_terms else ""
    return f"""
    MATCH (p:{PAPER_LABEL})
    WHERE p.title IS NOT NULL{filter_clause}
...
# Queries separadas para relaciones (sin cambios)
GET_AUTHORS_QUERY = f"""
...
def get_mmr_query(search_terms: List[str] = None, require_all: bool = True) -> str:
    filter_clause = build_filter_clause(search_terms, require_all) if search_terms else ""
    return f"""
    MATCH (p:{PAPER_LABEL})
    WHERE p.title IS NOT NULL
      AND p.abstract IS NOT NULL
      AND p.abstract <> ''{filter_clause}
    WITH p
    ORDER BY p.cited_by_count DESC
    LIMIT $limit
    OPTIONAL MATCH (p)-[:PUBLISHED_IN]->(j:{JOURNAL_LABEL})
...
def export_scopus_csv(drv, search_terms: List[str] = None, require_all: bool = True, 
                     hard_limit: Optional[int] = None) -> str:
    """
    ...
    Args:
        drv: Driver de Neo4j
        search_terms: Lista de términos a buscar (ej: ["circular economy", "plastic"])
        require_all: True = requiere TODOS los términos, False = al menos uno
        hard_limit: Límite máximo de papers a exportar
    """
    
    # Construir query de conteo
    count_query = get_count_query(search_terms, require_all)
    
    with drv.session(database=NEO4J_DB) as ses:
        total = ses.run(count_query).single()["total"]
    
    if total == 0:
        print(f"❌ No se encontraron papers con los términos: {search_terms}")
        return ""
    
    if hard_limit:
        total = min(total, hard_limit)

    # Construir nombre de archivo con términos
    terms_str = "_".join([t.replace(" ", "-") for t in search_terms]) if search_terms else "all"
    ts = now_stamp()
    out_path = os.path.join(OUT_DIR, f"scopus_{terms_str}_{ts}.csv")
    ...
    print(f"\n🔍 Filtros aplicados:")
    if search_terms:
        operator = "TODOS" if require_all else "AL MENOS UNO"
        print(f"   Términos: {search_terms}")
        print(f"   Modo: {operator}")
    else:
        print(f"   Sin filtros (todos los papers)")
    
    print(f"\n📊 Total registros encontrados: {total:,}")
    print(f"📦 Tamaño de lote: {PAGE_SIZE}")
    
    # Construir query paginada
    paged_query = get_paged_query(search_terms, require_all)
        ...
        with drv.session(database=NEO4J_DB) as ses:
            while exported < total:
                ...
                try:
                    ...
                    # Para cada paper, obtener relaciones
                    for paper in batch:
...
# ================== MMR (sin cambios significativos) ==================
...
def run_mmr(drv, search_terms: List[str] = None, require_all: bool = True,
            k: int = 50, method: str = "sbert", lam: float = 0.6) -> str:
    print(f"\n🔬 MMR → método={method}, k={k}, λ={lam}")
    
    mmr_query = get_mmr_query(search_terms, require_all)
    ...
    ts = now_stamp()
    terms_str = "_".join([t.replace(" ", "-") for t in search_terms]) if search_terms else "all"
    out_reps = os.path.join(OUT_DIR, f"mmr_{terms_str}_{method}_{ts}.csv")
    reps.to_csv(out_reps, index=False, encoding="utf-8")
    
    out_div = os.path.join(OUT_DIR, f"mmr_diversity_{terms_str}_{ts}.csv")
...
def main():
    print("\n" + "="*60)
    print("  EXPORTADOR OPENALEX → VOSviewer (OPTIMIZADO + FILTROS)")
    print("="*60)
    ...
    print(f"\n📁 Salida: {OUT_DIR}\n")
    print("Opciones:")
    print("1) Exportar CSV con FILTROS")
    print("2) Exportar CSV SIN filtros (todos)")
    print("3) MMR con filtros")
    print("4) Salir\n")
    ...
    try:
        ...
        if opt == "1":
            ...
            print("\n🔍 CONFIGURAR FILTROS:")
            print("   Ejemplo: circular economy, plastic")
            terms_input = input("   Términos a buscar (separados por coma): ").strip()
            
            if not terms_input:
                print("❌ No se ingresaron términos.")
                return
            
            search_terms = [t.strip() for t in terms_input.split(",")]
            
            print("\n   Modo de búsqueda:")
            print("   1) AND - Requiere TODOS los términos")
            print("   2) OR  - Requiere AL MENOS UNO")
            mode = input("   Selecciona (1-2, default 1): ").strip()
            require_all = mode != "2"
            
            lim = input("\n   Límite opcional de registros (ENTER = todo): ").strip()
            lim = int(lim) if lim else None
            
            export_scopus_csv(drv, search_terms, require_all, lim)
            
        elif opt == "2":
            # Exportar sin filtros
            lim = input("Límite opcional de registros (ENTER = todo): ").strip()
            lim = int(lim) if lim else None
            export_scopus_csv(drv, None, True, lim)
            
        elif opt == "3":
            # MMR con filtros
            print("\n🔍 CONFIGURAR FILTROS PARA MMR:")
            terms_input = input("   Términos a buscar (separados por coma): ").strip()
            ...
            mode = input("   Modo (1=AND, 2=OR, default 1): ").strip()
            require_all = mode != "2"
            ...
            lam = input("   λ (0..1, default 0.6): ").strip()
            lam = float(lam) if lam else 0.6
            
            run_mmr(drv, search_terms, require_all, k, method, lam)
